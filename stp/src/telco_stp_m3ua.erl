...
encode_param(error_code, invalid_version) ->
    encode_param(error_code, ?STP_M3UA_ERROR_INVALID_VERSION);
encode_param(error_code, unsupported_message_class) ->
    encode_param(error_code, ?STP_M3UA_ERROR_UNSUPPORTED_MESSAGE_CLASS);
encode_param(error_code, unsupported_message_type) ->
    encode_param(error_code, ?STP_M3UA_ERROR_UNSUPPORTED_MESSAGE_TYPE);
encode_param(error_code, unsupported_traffic_mode) ->
    encode_param(error_code, ?STP_M3UA_ERROR_UNSUPPORTED_TRAFFIC_MODE);
encode_param(error_code, unexpected_message) ->
    encode_param(error_code, ?STP_M3UA_ERROR_UNEXPECTED_MESSAGE);
encode_param(error_code, protocol_error) ->
    encode_param(error_code, ?STP_M3UA_ERROR_PROTOCOL_ERROR);
encode_param(error_code, invalid_stream_identifier) ->
    encode_param(error_code, ?STP_M3UA_ERROR_INVALID_STREAM_IDENTIFIER);
encode_param(error_code, management_blocking) ->
    encode_param(error_code, ?STP_M3UA_ERROR_MANAGEMENT_BLOCKING);
encode_param(error_code, asp_identifier_required) ->
    encode_param(error_code, ?STP_M3UA_ERROR_ASP_IDENTIFIER_REQUIRED);
encode_param(error_code, invalid_asp_identifier) ->
    encode_param(error_code, ?STP_M3UA_ERROR_INVALID_ASP_IDENTIFIER);
encode_param(error_code, invalid_parameter_value) ->
    encode_param(error_code, ?STP_M3UA_ERROR_INVALID_PARAMETER_VALUE);
encode_param(error_code, parameter_field_error) ->
    encode_param(error_code, ?STP_M3UA_ERROR_PARAMETER_FIELD_ERROR);
encode_param(error_code, unexpected_parameter) ->
    encode_param(error_code, ?STP_M3UA_ERROR_UNEXPECTED_PARAMETER);
encode_param(error_code, destination_status_unknown) ->
    encode_param(error_code, ?STP_M3UA_ERROR_DESTINATION_STATUS_UNKNOWN);
encode_param(error_code, invalid_network_appearance) ->
    encode_param(error_code, ?STP_M3UA_ERROR_INVALID_NETWORK_APPEARANCE);
encode_param(error_code, missing_parameter) ->
    encode_param(error_code, ?STP_M3UA_ERROR_MISSING_PARAMETER);
encode_param(error_code, invalid_routing_context) ->
    encode_param(error_code, ?STP_M3UA_ERROR_INVALID_ROUTING_CONTEXT);
encode_param(error_code, no_configured_as) ->
    encode_param(error_code, ?STP_M3UA_ERROR_NO_CONFIGURED_AS);
